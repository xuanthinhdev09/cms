...
def index():
	...
	response.view = 'calendar/content.html'	
	return dict(content=content)
...
def edit():
	...
	form=Process.cms.sqlform(tablename,table_id,default_show={'start_date':request.now,'start_time':'00:00:00'})
...
def publish():
	...
	if form.process().accepted:
		...
		try:
			...
			pass
		cache.ram.clear()
		redirect(URL(c='plugin_calendar',f='explorer.html',args=request.args[:3]),client_side=True)
	response.view = 'plugin_calendar/content.html'	
	return dict(content=form)
