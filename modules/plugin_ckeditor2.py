...
class CKEditor(object):
	def __init__(self, db, download_url=('static','uploads/ckeditor')):
		...
		url ='template/%s/uploads/ckeditor'%(Configsite().template)

		self.download_url=  ['static',url]
		...
		self.settings.download_url = self.download_url
    ...
    def handle_upload(self):
		...
		path = os.path.join(current.request.folder, 'static/uploads/ckeditor')
		if upload != None:
